...
class TestOpenAndCloseForm:
    ...
    @pytest.mark.xfail(reason="can't click gray BG on a side of the form (in cart does somehow work)")
    ...
    def test_if_form_can_be_opened_and_closed_with_close_button(self, browser):
        browser.implicitly_wait(5)
        main_page = MainPage(browser, link)
        main_page.open()
        main_page.should_be_main_page()
        ...
        form_page = FormPage(browser, link)
        form_page.should_be_form_page()
        form_page.close_form_with_close_button()
